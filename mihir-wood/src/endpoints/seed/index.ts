...
const baseAddressIndiaData: Transaction['billingAddress'] = {
  title: 'Mr.',
  firstName: 'Mihir',
  lastName: 'Patel',
  phone: '9876543210',
  company: 'Mihir Wood Sales',
  addressLine1: '123 SG Highway',
  city: 'Ahmedabad',
  state: 'Gujarat',
  postalCode: '380001',
  country: 'IN',
...
export const seed = async ({
  ...
  const customerUSAddress = await payload.create({
    ...
    data: {
      ...
      ...(baseAddressIndiaData as Address),
    },
  })
  ...
  const pendingTransaction = await payload.create({
    ...
    data: {
      ...
      status: 'pending',
      billingAddress: baseAddressIndiaData,
    },
  })
  ...
  const orderInCompleted = await payload.create({
    ...
    data: {
      ...
      currency: 'INR',
      customer: customer.id,
      shippingAddress: baseAddressIndiaData,
      items: [
        {
          product: productTshirt.id,
          variant: smallTshirtHoodieVariant.id,
